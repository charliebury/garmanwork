...
def map2CartCoords(rho,edensity_index,aCartesianVector,bCartesianVector,cCartesianVector):
    ...
    #cartesian position
    START = numpy.array(aCartesianVector)*(float(STARTPOSITION_ordered[0])/float(GRIDSAMPLING_ordered[0])) + numpy.array(bCartesianVector)*(float(STARTPOSITION_ordered[1])/float(GRIDSAMPLING_ordered[1])) + numpy.array(cCartesianVector)*(float(STARTPOSITION_ordered[2])/float(GRIDSAMPLING_ordered[2]))
