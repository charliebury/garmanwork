...
def manual_peak_searching(datedir,minpospeakdist):
    ...
    print colored('Code to delete unwanted entries in the difference map peak .ha file.','blue')
    ...
    print 'Do manual peak analysis now? ('+colored('y','green')+'/'+colored('n','red')+')'
    ...
    if response1 in ('y','Y','yes','YES','Yes'):
        ...
        for element in arraypdb_coords:
            ...
            print str(counter1)+': Diff peak #' + str(element[22:26]).strip() +' @ '+\
                  colored('x = ' + str(element[30:38]),'yellow') +\
                  colored(', y = ' + str(element[38:46]),'cyan') +\
                  colored(', z = '+ str(element[46:54]),'magenta') +\
                  colored(', with rmsd peak of ' + str(element[54:60]),'blue')

            print 'Nearest positive peak at: '+ str(minpospeakdist[counter1-1])[0:4]+ ' Angstrom'           
            print colored('Keep this peak?','red')
            ...
            if response2 in ('y','Y','yes','YES','Yes'):
                ...
                print colored('Peak kept in file!','green')
            else:
